...
void LayerNormForwardGpu(cudaStream_t stream, const int64_t num_instances, const int64_t norm_size,
  ...
  DirectLoad<T, ComputeType> load(x_ptr, norm_size);
  AffineStore<ComputeType, T, true, true> store(y_ptr, norm_size, gamma_ptr, beta_ptr);
  DispatchLayerNorm<decltype(load), decltype(store), ComputeType>(
      stream, load, store, num_instances, norm_size, epsilon,
      static_cast<ComputeType*>(mean), static_cast<ComputeType*>(inv_variance));
}

template void LayerNormForwardGpu<float>(cudaStream_t stream, const int64_t num_instances, const int64_t norm_size,
                        const double epsilon, const float* x_ptr, const float* gamma_ptr,
                        const float* beta_ptr, float* y_ptr, void* mean,
                        void* inv_variance);

template void LayerNormForwardGpu<__half>(cudaStream_t stream, const int64_t num_instances, const int64_t norm_size,
                        const double epsilon, const __half* x_ptr, const __half* gamma_ptr,
                        const __half* beta_ptr, __half* y_ptr, void* mean,
                        void* inv_variance);
